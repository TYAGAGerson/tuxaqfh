...
static unsigned char keyPressed[512];
static void getGLUTSpecialKeystroke ( int key, int, int )
{
  keyPressed[256+key]=1;
  lastGLUTKeystroke = 256 + key ;
}
static void getGLUTSpecialKeyUpstroke ( int key, int, int )
{
  keyPressed[256+key]=0;
}
...
static void getGLUTKeyUpstroke ( unsigned char key, int, int )
{
  keyPressed[key]=0;
}

int isKeyTrue ( int key)
{
  return (int)keyPressed[key];
}
...
void initWindow ( int w, int h )
{
  ...
  glutKeyboardUpFunc     ( getGLUTKeyUpstroke ) ;
  glutSpecialFunc        ( getGLUTSpecialKeystroke ) ;
  glutSpecialUpFunc      (getGLUTSpecialKeyUpstroke);
