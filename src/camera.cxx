...
{
  if(isKeyTrue('.')) {cam_speed += 0.3f ;
                      if ( cam_speed > 2.0f )
                      cam_speed = 2.0f ;}
  if(isKeyTrue(',')) {cam_speed -= 0.3f ;
                      if ( cam_speed < 0.1f )
                      cam_speed = 0.1f ;}
  //if(isKeyTrue(0x7f)){toggle_far    () ;}
  if(isKeyTrue(256+GLUT_KEY_UP)) {zoom ( -cam_speed )    ;}
  if(isKeyTrue(256+GLUT_KEY_DOWN )) {zoom (  cam_speed )    ;}
  if(isKeyTrue(256+GLUT_KEY_RIGHT)) {pan  ( -cam_speed )    ;}
  if(isKeyTrue(256+GLUT_KEY_LEFT)) {pan  (  cam_speed )    ;}
  if(isKeyTrue(256+GLUT_KEY_PAGE_UP)) {lift  ( cam_speed )    ;}
  if(isKeyTrue(256+GLUT_KEY_PAGE_DOWN)) {lift  ( -cam_speed )    ;}
