...
void drawHelpText ()
{
  ...
  drawDropShadowText ( "Move Tux with the Keyboard (Joystick is currently obsolete)", 18, 70, 280 ) ;

  if ( currState->cheat_mode )
  {
    drawDropShadowText ( "Cheat mode controls:"  , 12, 150, 240 ) ;
    drawDropShadowText ( "x   -- Exit Cheat Mode.",10, 160, 220 ) ;
    drawDropShadowText ( "(   -- Console Mode. DO NOT PRESS"  , 10, 160, 210 ) ;
    drawDropShadowText ( "*   -- Texture."       , 10, 160, 200 ) ;
    drawDropShadowText ( "&   -- Wireframe."     , 10, 160, 190 ) ;
    drawDropShadowText ( "^   -- Freeze-Baddies.", 10, 160, 180 ) ;
    drawDropShadowText ( "%   -- Beginner Pack.", 10, 160, 170 ) ;
    drawDropShadowText ( "??  -- SuperTux."      , 10, 160, 160 ) ;
    drawDropShadowText ( "??  -- RocketPack."    , 10, 160, 150 ) ;
    drawDropShadowText ( "??  -- LaserGun."      , 10, 160, 140 ) ;
  }
  else
  /*{
...
void drawTitleText ()
{
  ...
  if ( play_as_gown )
    drawDropShadowText ( "Gown T. Penguin (who is Penny?)", 20, 70, 300 ) ;
  else
...
{ "Original Game:",
  ...
  "Modified by: TyagaGerson and contributors",
  NULL
} ;
