...
#define ROLL_COMBO     (2+8)

#define LEFT_BUTTON    32
#define RIGHT_BUTTON   16
#define CAMERA_BUTTON   64
...
static int prefer_joystick = FALSE ;
...
static int error=0;
/*static void checkwarpfn( int x, int y ){
  
  if(checkTimer){
    if(checkTimer>60){
      glutPassiveMotionFunc(motionfn);glutMotionFunc(motionfn);
      if(error>30){assert(0);}
      return ;
    }
    printf("well, %i\n%i\n%i\n\n",checkTimer,x,y);
    if(x!=checkTimer%2 || y!=checkTimer%2){
      
      fprintf(stderr,"mouse is not supported !!!!");
      error++;
      }
    glutWarpPointer(checkTimer%2, checkTimer%2); 
  }
  else glutWarpPointer(1,1);
  checkTimer++;
}*/
...
#ifdef /*SOME_BETTER_WAY_TO_DO_THIS*/ SOME_OLD_WAY_TO_DO_THIS
...
#else
  static void setFullscreen () {}
  static void window_off_cb ( puObject * ) { mousetrap = TRUE  ; glutFullScreen() ; } 
  static void window_on_cb  ( puObject * ) { mousetrap = FALSE ; glutReshapeWindow(640,480);glutPositionWindow(0,0) ; } 
#endif
...
static void joystick_off_cb  ( puObject * ) { prefer_joystick = FALSE ;  } 
...
#endif
#endif
char* env=std::getenv("XDG_SESSION_TYPE");
if(strcmp(env, "wayland")){
  mousetrap=TRUE;
  glutSetCursor(GLUT_CURSOR_NONE);
}else{
  printf("WARNING: glut may doesn't fully support wayland, some settings/controls (mouse warping/mousetrap) are changed to accommodate gameplay. (though you can change the mousetrap setting in menu if you still dare to)");
}
  ...
  glutWarpPointer(10, 10);
  glutWarpPointer(20, 10);
  puInit () ;
...
void GUI::joystickInput ()
{
  ...
  if ( isHidden() && ( joystick->notWorking() || ! prefer_joystick ) )
  {
    ...
    */
    ...
    if ( mousetrap && ( ji.buttons & 0x04 ) != ( mouse_buttons & 0x04 ) )
    {
      mouse_dx = 0 ;
      mouse_dy = 0 ;
    }
    ...
    ji.buttons|= LEFT_BUTTON*isKeyTrue('[');
    ji.buttons|= RIGHT_BUTTON*isKeyTrue(']');
    ji.buttons|= CAMERA_BUTTON*isKeyTrue(';');
    ji.buttons|= CAMERA2_BUTTON*isKeyTrue('\'');
    CD_hack=isKeyTrue('r');
    ji.buttons|= JUMP_BUTTON*isKeyTrue('h');
    ji.buttons|= FIRE_BUTTON*isKeyTrue('j');

    ji.data[0] = (float)mouse_dx / mscale ;
    ji.data[1] = (float)mouse_dy / mscale ;
    ji.data[0] += (float) (isKeyTrue('d')-isKeyTrue('a'))*100000.;
    ji.data[1] += (float) (isKeyTrue('s')-isKeyTrue('w'))*100000.;
  ...
  #ifdef DEBUGGING
  if(currState -> cheat_mode&&!(getClock()%10)){
  cout << "mouse dx: " << mouse_dx;
  cout << "mouse dy:" << mouse_dy;
  cout << "stick w diff: " << ji.data[0];
  cout << "stick h diff" << ji.data[1];
  cout << flush;}
  #endif
  }
  else
  {
    mouse_dx = mouse_dy = 0 ;
    mouse_buttons = 0 ;
    if((!joystick->notWorking()) && prefer_joystick){
    joystick -> read ( & ji.buttons, ji.data ) ;}
  }

  if ( CD_hack )
  {
    ji.buttons |= 2 + 8 ;  /* Set the C+D buttons - kludhe a 'ROLL'. */
    CD_hack = FALSE ;
  }
